...
exports.route = function(req, res) {
	...
	var describeQuery = 'select column_name, data_type from information_schema.columns where table_schema = \''+process.env.HEROKU_CONNECT_SCHEMA+'\' and table_name=\''+req.params.table_name+'\'';
	...
	dataAccess.query(describeQuery, function(objectDescribe, describeSuccess, describeErr) {
		...
		if(describeSuccess) {
			...
			dataAccess.query(dataQuery, function(dataRows, success, err) {
				...
				res.render('table', {records: records, page_heading: 'Heroku Connect Demo'} ); 
			});
		} else {
			console.error('There was an error querying the database\n', err);
			res.render('table', {records: records, page_heading: 'Heroku Connect Demo'} ); 
